...
func test_map_normalized_coordinate_to_screen() -> void:
	...
	var expected := Vector2(
		screen_pos.x + 0.5 * screen_size.x,
		screen_pos.y + 0.5 * screen_size.y
	)
	var mapped := NaviUtils.map_normalized_coordinate_to_screen(norm_pos)
	assert_eq(mapped, expected, "Coordinates should be correctly mapped based on screen size and position.")
